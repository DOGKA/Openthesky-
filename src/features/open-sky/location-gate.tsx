...
export function OpenSkyLocationGate({ status, t, onRequest, onFallback, onRetry }: Props) {
      ...
      <View style={{ gap: 10 }}>
        {denied ? (
          <>
            ...
            />
            <GateButton label={t("loc_fallback")} onPress={onFallback} secondary />
          </>
        ) : (
          <GateButton
...
function GateButton({
  ...
  return (
    ...
    >
      ...
      <MonoText size={12} medium upper tracking={1} style={{ color: secondary ? SKY.fg70 : SKY.bg }}>
        {label}
      </MonoText>
    </Pressable>
  );
